...
def find_build(app: str, platform: str, build_id: str | None) -> dict:
    ...
    if not mine:
        raise SystemExit(
            f"no {platform} builds for this app yet. Push a v* tag — the "
            f"Mobile Build workflow archives, signs and uploads the IPA."
        )
...
def main() -> int:
    ...
    if not app:
        raise SystemExit(
            ...
            "including Admin) — it has to be done in the browser."
        )
