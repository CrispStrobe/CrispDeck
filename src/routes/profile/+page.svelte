...
<div class="p-6 max-w-3xl mx-auto">
  ...
  {:else if error}
    ...
    <button onclick={() => history.back()} class="flex items-center gap-1 text-sm text-[var(--color-primary-text)]"><ArrowLeft size={14} />{i18n.t.profile.backToFeed}</button>
  {:else if profile}
    <!-- Banner -->
    {#if profile.banner}
    ...
    {#if activeTab === 'followers' || activeTab === 'following'}
      ...
      {:else}
        <div class="space-y-1">
          {#each list as user}
            <a href="{base}/profile?handle={encodeURIComponent(user.handle)}&platform={platform}" class="flex items-center gap-3 p-2.5 rounded-lg hover:bg-[var(--color-surface-hover)] transition-colors">
              {#if user.avatar}
