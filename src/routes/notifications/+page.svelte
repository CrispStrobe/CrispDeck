...
<div class="p-6 max-w-3xl mx-auto" role="feed" aria-label={i18n.t.nav.notifications} ontouchstart={ptr.onTouchStart} ontouchmove={ptr.onTouchMove} ontouchend={ptr.onTouchEnd}>
  {#if ptr.state.pullDistance > 0}
    <div class="flex items-center justify-center mb-2 transition-all" style="height: {ptr.state.pullDistance}px">
      <RefreshCw size={20} class="text-[var(--color-primary-text)] {ptr.state.pullRefreshing ? 'animate-spin' : ''}" style="opacity: {ptr.state.pullDistance / 60}; transform: rotate({ptr.state.pullDistance * 3}deg)" />
    </div>
  {/if}
