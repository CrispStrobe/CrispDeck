      ...
      {#if showCreate}
        <div class="p-4 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)] space-y-3">
          ...
          <div class="flex gap-2">
            ...
            <button onclick={() => showCreate = false} class="px-4 py-2 text-sm text-[var(--color-text-muted)] border border-[var(--color-border)] rounded-md">{i18n.t.common.cancel}</button>
          </div>
        </div>
      {:else}
