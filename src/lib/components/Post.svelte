      ...
      {#if bskyImages.length > 0}
        <div class="{bskyImages.length === 1 ? '' : 'grid grid-cols-2 gap-2'} pt-2">
          {#each bskyImages as image, i}
            ...
            >
              <img src={image.thumb} alt={image.alt || ''} class="rounded-md w-full {bskyImages.length === 1 ? 'aspect-video object-cover' : 'aspect-square object-cover'}" />
              {#if image.alt}
                <span
          ...
          {#each mastodonMedia as attachment, i}
            ...
            {#if imageUrl}
              <button
                ...
                class="cursor-pointer text-left w-full relative"
              >
                <img src={imageUrl} alt={attachment.description || `Image ${i + 1}`} class="rounded-md w-full {mastodonMedia.length === 1 ? 'aspect-video object-cover' : 'aspect-square object-cover'} bg-[var(--color-surface-hover)]" />
