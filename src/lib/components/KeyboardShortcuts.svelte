<script lang="ts">
  ...
  $effect(() => {
    ...
    dialogEl.focus();
    const focusable = dialogEl.querySelectorAll<HTMLElement>('button, a, [tabindex]:not([tabindex="-1"])');
...
{#if show}
  ...
  <div bind:this={dialogEl} class="fixed inset-0 z-[100] flex items-center justify-center" role="dialog" aria-modal="true" aria-label="Keyboard shortcuts" tabindex="-1" onclick={() => show = false} onkeydown={(e) => { if (e.key === 'Escape') show = false; }}>
